...
class Collection extends IlluminateCollection
{
	...
	public function whereLanguage($value){
    	return $this->filter(function ($data) use ($value){
    		if(isset($data->languages))
    			return in_array($value, (array) $data->languages);
			return false;
		});
	}
	...
	public function whereISO639_3($value){
		return $this->filter(function ($data) use ($value){
			if(isset($data->languages))
				return Arr::has($data->languages, $value);
			return false;
		});
	}
	...
	public function whereISO4217($value){
		return $this->filter(function ($data) use ($value){
			if(isset($data->currency))
				return in_array($value, $data->currency);
			return false;
		});
	}
